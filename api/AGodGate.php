...
class AGodGate {
    ...
    function changeGateUnlockedCells() {
        ...
        for ($iii = 1; $iii <= 4; $iii++) {
            ...
            $unlockedTyps = array_diff(["vit", "attack", "defence", "damage", "break", "anti_break", "strike", "immunity"], $lockedTypes);
    ...
    function OpenFourthCell() {
        ...
        if (!is_numeric($GateIndex))
            return ["state" => "error_0"];
        if ($GateIndex > 4 || $GateIndex <= 0)
            return ["state" => "error_1"];

        $Gate = selectFromTable("*", "god_gate_" . $GateIndex, "id_player = :idp", ["idp" => $idPlayer]);
        if (!count($Gate))
            return ["state" => "error_2"];
        if($godGateData[0]["points"] < $GateIndex*5000)
            return ["state" => "error_3"];
        if(!LItem::useItem("skill_book", $GateIndex*10))
            return ["state" => "error_4"];
        
        
        $newPoint = ["break", "anti_break", "strike", "immunity"];
        updateTable("c_4_s = 1, cell_4_score = 0, cell_4_type = :t", "god_gate_" . $GateIndex, "id_player = :idp", ["idp" => $idPlayer, "t" => $newPoint[rand(0, 3)]]);
        updateTable("points  = points - :p", "god_gate", "id_player = :idp", ["idp" => $idPlayer, "p" => $GateIndex*1500]);
