...
class APlayer {
    ...
    function getAllNotif() {
        global $idPlayer;
        $Q = queryExe("SELECT
                            (SELECT COUNT(*) FROM msg_income WHERE msg_income.id_to = $idPlayer AND seen = 0) AS msg_in,
                            (SELECT COUNT(*) FROM msg_diff WHERE msg_diff.id_to = $idPlayer AND seen = 0) AS msg_diff,
                            (SELECT COUNT(*) FROM report_player WHERE report_player.id_player = $idPlayer AND seen = 0) AS msg_report,
                            (SELECT COUNT(*) FROM spy_report WHERE spy_report.id_player = $idPlayer AND seen = 0) AS spy_report,
                            (SELECT COUNT(*) FROM battel WHERE id_player = $idPlayer) AS battel_number ,
                            (SELECT COUNT(*) FROM battel_member WHERE id_player = $idPlayer) AS hero_in_battel, 
                            (SELECT COUNT(*) FROM hero_back WHERE id_player = $idPlayer) AS hero_back ,
                            (SELECT COUNT(*) FROM spy  WHERE id_player = $idPlayer) AS spy_task 
                            ");
        $notif = $Q["Rows"][0];
        ...
        $notif ["battel_number"] += selectFromTable("COUNT(DISTINCT battel.id_battel) AS count", $quary, "world_unit_garrison.id_player = $idPlayer")[0]["count"];
        ...
        $notif ["battel_number"] += selectFromTable("COUNT(DISTINCT battel.id_battel) AS count",
                        ...
                        "city.id_player = $idPlayer")[0]["count"];
        return $notif;
    }
