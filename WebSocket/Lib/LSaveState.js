class LSaveState {
    ...
    static  saveCityState(idCity, callBack)
    {
        var now = Math.floor(Date.now() / 1000);

        Elkaisar.DB.Update(`coin = GREATEST( LEAST( coin   + (CAST(coin_in   AS SIGNED) - CAST(coin_out   AS SIGNED))*(${now} - LS)/3600 , GREATEST( coin_cap , coin )  ) , 0), 
                 ...
                 LS = ${now} `, "city", "id_city = ?", [idCity], function (Res) {
            if (callBack)
                callBack(Res);
        });
